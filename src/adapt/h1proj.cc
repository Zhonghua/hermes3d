...
void H1Projection::calc_vertex_proj(int split, int son) {
	_F_
	int nv = base_elem->get_num_of_vertices();
	...
	for (int i = 0; i < edge_fns; i++) {
		...
		for (int j = 0; j < edge_fns; j++) {
			...
			if (iedge == 0 || iedge == 2 || iedge == 8 || iedge == 10) {
				...
				EXIT(ERR_FAILURE, "local edge number out of range.");
...
void H1Projection::calc_face_proj(int iface, int split, int son, order3_t order) {
	...
	int ipol_fns = RefHex::get_face_num_of_vertices(iface);
	for (int iedge = 0; iedge < RefHex::get_face_num_of_edges(iface); iedge++)
		ipol_fns += order.get_edge_order(face_edge[iedge]) - 1;

	// interpolant
	ProjItem ipol[ipol_fns];
	int mm = 0;
	for (int vtx = 0; vtx < RefHex::get_face_num_of_vertices(iface); vtx++, mm++)
	...
	for (int i = 0; i < face_fns; i++) {
		...
		for (int j = 0; j < face_fns; j++) {
			...
			if (iface == 0 || iface == 1) {
				...
				EXIT(ERR_FAILURE, "Local face number out of range.");
