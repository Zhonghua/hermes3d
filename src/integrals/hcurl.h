...
// for surface integrals : unit normal x u x unit normal ... tangent projection of u
...
// for surface integrals : unit normal x v x unit normal ... tangent projection of v
...
inline scalar hcurl_int_F_u(scalar3 &(*F)(double x, double y, double z), RealFunction *fu, RefMap *ru) {
	...
	if (ru->is_jacobian_const()) {
		mu = ru->get_const_inv_ref_map();
		for (int i = 0; i < np; i++) {
			scalar3 &f = F(x[i], y[i], z[i]);
			result += pt[i].w * (T_U_0 * f[0] + T_U_1 * f[1] + T_U_2 * f[2]);
		}
		result *= ru->get_const_jacobian();
	}
	else {
		...
		for (int i = 0; i < np; i++, mu++) {
			scalar3 &f = F(x[i], y[i], z[i]);
			result += pt[i].w * jac[i] * (T_U_0 * f[0] + T_U_1 * f[1] + T_U_2 * f[2]);
		}
	}
...
inline scalar hcurl_surf_int_G_v(RealFunction *fv, RefMap *rv, FacePos *fp) {
	...
	if (rv->is_face_const_jacobian(fp->face)) {
		...
		mv = rv->get_face_const_inv_ref_map(fp->face);
		for (int i = 0; i < np; i++) {
			scalar3 &f = fp->space->bc_vec_value_callback_by_coord(fp->marker, x[i], y[i], z[i]);
			result += pt[i].w * (N_X_V_X_N_0 * f[0] + N_X_V_X_N_1 * f[1] + N_X_V_X_N_2 * f[2]);
		}
		result *= rv->get_face_const_jacobian(fp->face);
	}
	else {
		...
		nv = rv->get_face_normal(fp->face, face_order);
		mv = rv->get_face_inv_ref_map(fp->face, face_order);
		for (int i = 0; i < np; i++, nv++, mv++) {
			scalar3 &f = fp->space->bc_vec_value_callback_by_coord(fp->marker, x[i], y[i], z[i]);
			result += pt[i].w * jac[i] * (N_X_V_X_N_0 * f[0] + N_X_V_X_N_1 * f[1] + N_X_V_X_N_2 * f[2]);
		}
	}
