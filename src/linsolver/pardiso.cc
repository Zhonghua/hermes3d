...
void PardisoMatrix::set_zero() {
	_F_
    memset(Ax, 0, sizeof(scalar) * Ap[size]);
}

void PardisoMatrix::update(int m, int n, scalar v) {
	_F_
	insert_value(Ai + Ap[m], Ax + Ap[m], Ap[m + 1] - Ap[m], n, v);
}

void PardisoMatrix::update(int m, int n, scalar **mat, int *rows, int *cols) {
	_F_
...
void PardisoVector::update(int idx, scalar y) {
	_F_
	if (idx >= 0) v[idx] += y;
}

void PardisoVector::update(int n, int *idx, scalar *y) {
	_F_
	for (int i = 0; i < n; i++)
