...
Vector3D transform (Mesh *mesh, RefMap *rm, Vector3D orig)
{
	...
	trans.z = (*m)[2][0] * orig.x + (*m)[2][1] * orig.y + (*m)[2][2] * orig.z;
...
//
bool test_cont_values_of_edge_fns(Mesh *mesh, int pos0, int pos1, Shapeset *shapeset, RefMap* rm0, RefMap* rm1) {
...
// @return false 	if fn. values do not match
//
bool test_cont_values_of_face_fns(Mesh *mesh, int pos0, int pos1, Shapeset *shapeset, RefMap* rm0, RefMap* rm1) {
	Quad3D *quad = get_quadrature(MODE);
...
void add_facets(MapOrd<Facet *> &facets, Element *hex) {
	// facets
...
bool test_continuity(Shapeset *shapeset) {
	...
	for (int i = 0; i < 24; i++) {
		for (int j = 0; j < 24; j++) {
			...
			if (!test_cont_values_of_edge_fns(&mesh, i, j, shapeset, &rm0, &rm1))
				return false;

			if (!test_cont_values_of_face_fns(&mesh, i, j, shapeset, &rm0, &rm1))
				return false;
		}
	}
